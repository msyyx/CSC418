...
function drawPolygon(ctx, poly)
{
  ...
  x1 = poly[poly.length - 1][0];
  x2 = poly[0][0];
  y1 = poly[poly.length - 1][1];
  y2 = poly[0][1];
  drawLineSegment(ctx, x1, y1, x2, y2);  
}
...
function drawPenguin(ctx)
{
  ...
  // Draw the arm
  var arm_offset = [15, -30];
  var arm_joint_offset = [0, -25];
  ...
  // Draw the lower beak
  var lower_beak_offset = [-90, mouth_gap + 31];
  ...
  var left_leg_offset = [-70, 180];
  var left_leg_joint_offset = [0, -30];
  var left_leg_T = composeTransforms(
    rotationAboutPoint(hip_angles[0], left_leg_joint_offset),
  ...
  hip_joints[0] = transformPoint(left_leg_joint_offset, left_leg_T);
  ...
  var right_leg_joint_offset = [0, -30];
  var right_leg_T = composeTransforms(
    rotationAboutPoint(hip_angles[1], right_leg_joint_offset),
  ...
  hip_joints[1] = transformPoint(right_leg_joint_offset, right_leg_T);
